...
public class Test0914
{
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		double A1 = 17.53, C_storm = 0.95, tmin = 10, b_storm = 11.77, P_simu = 1, n_storm = 0.88, dt = 2.0, rc = 0.375, Hw_end = 3.0, vp0 = 0.8, csf = 3.0;
		// 节点汇水面积(ha)
		...
		double Ad0, qkpmax, Hwdwkp, yykp, sita, cons_b, sita_s, sita_c, fsita, dfdsita, dfsita, ssita, csita, hyd_A, hafsita, shafsita, chafsita, sita_p;
		// 中间变量
		...
		String FileName = "Test0914-雨水管网过程模拟-芝加哥过程线-华家池002.txt";
		...
		// --输出数据文件开始---
		// ofstream outfile;
		// outfile.open("20160913-雨水管网过程模拟-华家池-1-3.txt");
		//
		// cout<<"------ 雨水管网过程模拟-华家池 ------"<<endl;
		System.out.print("------ 雨水管网过程模拟-华家池 ------");
		// ================= 赋初值 ===============================
		for (i = 0; i < NT; i++)
		{
			for (j = 0; j < NN; j++)
		...
		for (j = 0; j < NN; j++)
		{
			...
			{ // s.Format("%6.2f",Tnode[i][j]);
				// outfile<<s;
				printStream.printf("%6.2f", Tnode[i - 1][j]);
			}
		...
		{// outfile<<" it="<<it<<"  qpt[it][k]=";
				...
				// --3
				{
					...
					// --4
					{
						...
						if (Hwdw[it][kp] >= hdj0)
						{
							...
							{// outfile<<"   it= "<<it<<"   kp= "<<kp<<"  Hwdm= "<<Hwdw[it][kp]<<"  淹没出流 "<<endl;
						...
						// --5
						{
							...
							{
								if (Iprt == 1)
								{// outfile<<"   it= "<<it<<"   kp= "<<kp<<"  Hwdm= "<<Hwdw[it][kp]<<"  非淹没非满管出流 "<<endl;
								...
								if (Iprt == 1)
								{// outfile<<"   k= "<<k<<"   qpt[it][kp]= "<<qpt[it][kp]<<"   cons_b= "<<cons_b<<endl;
								...
								while (true)
								{
									...
									else
									{
										...
										if (Iprt == 1)
										{// outfile<<"   i= "<<i<<"   sita= "<<sita<<"   ssita= "<<ssita<<"   csita= "<<csita<<"   fsita= "<<fsita<<"   dfdsita= "<<dfdsita<<endl;
								...
								//
								/*
								 * L100 :hdcc0[it][kp]=(1-Math.cos(sita/2))/2;
								 * rid
								 * [it][kp]=0.25*dpl[kp]*(sita-Math.sin(sita))
								 * /sita;
						...
						if (Iprt == 1)
						{/*
						 * outfile<<"   it= "<<it<<"   kp= "<<kp<<"   I0[kp]= "<<
						 * I0
						 * [kp]<<"  Hwdm= "<<Hwdw[it][kp]<<"  Hwup= "<<Hwup[it]
						 * [kp]<<"  Hj= "<<Hj[I0[kp]]
						 * <<"  hdcc0= "<<hdcc0[it][kp
						 * ]<<"  qpt= "<<qpt[it][kp]<<
						 * "  qqkp= "<<qqkp[it][kp]<<"  vpt= "
			...
			for (i = 0; i < NP; i++)
			{// s.Format("%6d%6d%6d%5d%8.2lf%12.3lf%10.3lf%8.3lf%10.3lf%10.3lf%10.3lf%10.3lf%10.3lf%10.5lf%10.3lf\n",
				// it,i,I0[i],J0[i],dpl[i],qpt[it][i],rid[it][i],hdcc0[it][i],vpt[it][i],Hwup[it][i],Hwdw[it][i],ZJup[i],ZJdw[i],slop[i],Hj[I0[i]]);
				// outfile<<s;

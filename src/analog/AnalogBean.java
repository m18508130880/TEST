...
public class AnalogBean {
	...
	public void ExecCmd() throws FileNotFoundException {
		...
		String FileName = "201608030-雨水管网过程模拟-芝加哥过程线.txt";
	...
	private double[] Aj; // 节点汇水面积(ha)(double)[]
	private double[] Acoef; // 节点汇水面积径流系统(double)[]
	private double[] Hj; // 节点地面标高（m）[NN=23] (double)[]
	private int[][] Mroute; // 管网路径数和路径节点号(－99表示空节点)(int)[][]
	private int[][] Mbranch; // 节点汇水面积径流系统(int)[][]
	private int[] I0; // 管段上游节点号I0(int)[]
	private int[] J0; // 下游节点号J0(int)[]
	private double[] lp; // 管段长度(m)(double)[]
	private double[] slp; // 摩阻系数(double)[]
	private double[] dpl; // 管段直径(m)(double)[]
	private double[] ZJup; // 上游管底高程(m)(double)[]
	private double[] ZJdw; // 下游管底高程(m)(double)[]
